# database.py (Versão com novas funções de admin)
"""
    ...
    with get_db_connection() as conn:
        try:
            ...
            if cursor.rowcount > 0:
                conn.commit()
                # Cria um registro da transação de ajuste
                record_transaction(
                ...
                return False # Nenhum usuário com esse ID foi encontrado
        except sqlite3.Error as e:
            logger.error(f"❌ Erro no banco de dados ao setar saldo para {user_telegram_id}: {e}", exc_info=True)
            conn.rollback()
            return False
...
def get_users_with_balance():
    ...
    Ordenado do maior saldo para o menor.
    """
    with get_db_connection() as conn:
        try:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT telegram_id, first_name, username, balance FROM users WHERE balance > 0 ORDER BY balance DESC"
            )
...
# ... (o resto do seu arquivo database.py continua aqui, sem alterações)
